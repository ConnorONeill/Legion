...
if(room == rm_battle){
	...
	if (global.area != noone){
		if (global.enemy == noone and global.area[global.level-1] != noone){
			global.enemy = instance_create_depth(room_width/2+10,room_height/2, 10, global.area[global.level-1]);	
	//if there is no enemy to replace move on to next area, can put all things assosiated to finishing area here
		}else if (global.enemy == noone and global.area[global.level-1] == noone){
